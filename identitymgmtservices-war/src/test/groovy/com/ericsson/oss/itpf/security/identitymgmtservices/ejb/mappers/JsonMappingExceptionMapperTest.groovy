...
class JsonMappingExceptionMapperTest extends CdiSpecification {

    @ObjectUnderTest
    JsonMappingExceptionMapper jsonMappingExceptionMapper

    def JsonMappingException exception = Mock()

    def "build toResponse"() {
        given:

        when: "building toResponse"
        Response response = jsonMappingExceptionMapper.toResponse(exception)
